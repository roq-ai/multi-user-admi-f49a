...
import { getCoupons, deleteCouponById } from 'apiSdk/coupons';
import { CouponInterface } from 'interfaces/coupon';

type ColumnType = ColumnDef<CouponInterface, unknown>;
...
export function CouponListPage(props: CouponListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CouponInterface>>(
    () => `/coupons?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CouponInterface) => {
    if (hasAccess('coupon', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/coupons/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/coupons/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('coupon', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/coupons/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('coupon', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="coupon" />
        </Flex>

        {hasAccess('coupon', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/coupons/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}

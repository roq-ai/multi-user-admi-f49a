...
import { getCoupons } from 'apiSdk/coupons';
import { CouponInterface } from 'interfaces/coupon';

import { TeamListPage as ShowCaseList } from 'pages/teams';
...
function DashboardPage() {
  ...
  const couponStat = useSWR<PaginatedInterface<CouponInterface>>(
    () => `/coupons`,
    () => getCoupons({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Coupons',
      result: couponStat?.data?.totalCount,
      isLoading: couponStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
